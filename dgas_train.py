...
CONFIG = {
    ...
    "SAVE_INTERVAL": 1000,
    "BATCH_SIZE": 8,       # Ajustado para caber na memória (sem crop)
    "STEPS": 1000000,
    "WARMUP_STEPS": 5000, 
...
def gpu_stft(audio):
    ...
    # --- CORREÇÃO CRÍTICA ---
    # Removido o corte [:128] e o multiplicador * 10.0
    # O modelo agora vê o espectro inteiro (1025 bins) e usa a escala natural (log1p implícito pela normalização do data loader)
    return spec # Retorna Full Frequency (aprox 1025 bins)
...
def diff_spec_augment(x, key, strength):
    ...
    # Ajustado para ser relativo ao tamanho F
    f_width = int(F * 0.15) 
...
def compute_disc_loss(discriminator, generator, mix_spec, target_spec, key, do_r1, aug_strength):
    ...
    def predict_batch_sample(ti, xti, zi):
        ...
        # Passar F, T individualmente para o field
...
# --- 4. STEP DE TREINO (Igual ao original, apenas assinatura mantida) ---
@eqx.filter_jit
...
# --- MAIN ---
def main():
    print(f"=== DGAS 3.6.2: FULL SPECTRUM FIX & IMPROVEMENTS ===")
    ...
    lr = optax.warmup_cosine_decay_schedule(1e-5, 2e-4, 2000, CONFIG["STEPS"], 1e-6)
    ...
    try:
        while step < CONFIG["STEPS"]:
            ...
            if step % 10 == 0:
                print(f"S{step:05d} | G:{g_loss:.2f} D:{d_loss:.2f} | FL:{flow:.2f} MRS:{mrs:.2f} PH:{phase:.3f} | R1:{r1:.1f} ADA:{pid_state[1]:.2f} | {dt*1000:.0f}ms")
